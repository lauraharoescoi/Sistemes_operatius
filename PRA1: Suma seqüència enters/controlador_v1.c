...
int main(int argc, char *argv[]) {
    long int i;
    int sumTotal = 0;       //variable que acumula el valor de la suma parcial de cada calculador
    char *args[] = {"./calculador_v1", "./calculador_v1", "1", "20"};
    pid_t pid;
    int estatWait;
    char *nomPrograma;
    char cadena[STRLONG];
    ...
    for (i = 0; i < numCalculadors; i++) {
        ...
        sumTotal += estatWait >> 8;     //Afegim el valor obtingut a través del wait a la variable sumTotal
        write(1, cadena, strlen(cadena));
    }

    long long int sumForm = ((terme_1 + terme_n) / 2.0) * (terme_n + 1 - terme_1);

    sprintf(cadena, "%s%s - %d> Suma amb caluladors=%i - Suma amb formula=%.0lld%s\n", color_blue, nomPrograma, getpid(), sumTotal, sumForm, color_end);
    write(1, cadena, strlen(cadena));
